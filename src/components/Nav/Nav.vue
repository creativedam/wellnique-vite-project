<template>
  <div class="relative z-50 mx-auto grid w-full grid-cols-6 justify-between border-white bg-white px-4 py-2 transition-all duration-300 md:px-10">
    <!-- MOBILE BURGER BUTTON -->
    <button @click="mobileOpen = !mobileOpen" class="text-org-800 flex h-25 items-center text-3xl font-bold sm:hidden">☰</button>
    ...
    <div class="col-span-6 mx-auto hidden w-full justify-center sm:block md:container">
      <nav id="menu-main" class="flex w-full justify-center">
        <ul id="nav-main" class="flex w-7/8 justify-center font-medium text-slate-700">
          <li class="py-4">
            <router-link class="hover:text-org-400 hidden px-5 py-2 text-center text-sm leading-tight font-semibold drop-shadow-md lg:block lg:text-lg" to="/">HOME</router-link>
          </li>
          <li v-for="(menu, key) in menus" :key="key" class="relative my-4" @mouseenter="activeMenu = key">
            <router-link class="hover:text-org-400 mx-1 block px-5 py-2 text-center text-sm font-semibold md:mx-2 lg:mx-5 lg:px-5 lg:text-lg" :to="menu.url">{{ menu.title }}</router-link>
          </li>
          <li class="my-4">
            <router-link class="hover:text-org-400 mx-2 block px-5 py-2 text-center text-sm font-semibold lg:text-lg" to="/contacts">CONTACT</router-link>
          </li>
        </ul>
      </nav>

      <!-- DROPDOWN PANELS -->
      <div v-for="(menu, key) in menus" :key="key" class="absolute top-21 right-0 left-0 mt-2 w-full" @mouseenter="activeMenu = key" @mouseleave="closeMenus">
        <div v-if="activeMenu === key" class="animate__animated animate__fadeIn z-30 grid min-h-60 w-full grid-cols-3 overflow-hidden drop-shadow-md">
          <div class="menu-title flex items-center justify-center border-b-2 border-b-slate-200 bg-slate-300/95 text-black">
            <div class="relative text-center">
              <h1 class="text-4xl drop-shadow-md">{{ menu.title }}</h1>
              <span class="bg-org-400 absolute left-1/2 mt-2 block h-2 w-22 -translate-x-1/2"></span>
            </div>
          </div>
          <div class="flex flex-col justify-center border-b-2 border-b-white bg-white/95 px-8 py-5 text-gray-700">
            <ul class="animate__animated animate__fadeInDown">
              <li v-for="item in menu.items" :key="item.key" class="mb-2 block w-full cursor-pointer">
                <router-link class="hover:text-org-600 block border-transparent py-1 pl-4 transition hover:border-l-4" :to="item.url" @mouseover="activeTab = item.key">
                  {{ item.label }}
                </router-link>
              </li>
            </ul>
          </div>
          <div class="flex flex-col justify-center border-b-2 border-b-white bg-white/95 px-8 py-5 text-left text-gray-700">
            <div v-for="item in menu.items" :key="item.key" v-show="activeTab === item.key" class="animate__animated animate__fadeInDown pointer-events-none p-3">
              {{ item.description }}
            </div>
          </div>
        </div>
    ...
    <transition name="slide">
      <div v-if="mobileOpen" class="animate__animated animate__fadeInLeft fixed inset-y-0 left-0 z-50 w-64 overflow-y-auto bg-gray-600 p-6 shadow-lg sm:hidden">
        <button @click="mobileOpen = false" class="bg-grey-500 mb-4 border border-white px-4 py-2 text-2xl font-bold text-gray-600 hover:bg-white hover:text-gray-600">×</button>
        <ul class="space-y-4">
          <li>
          ...
          <li v-for="(menu, key) in menus" :key="key" class="border-b pb-2">
            ...
            <ul v-show="mobileDropdown === key" class="space-y-2 pl-4">
              <li v-for="item in menu.items" :key="item.key">
                <router-link @click="mobileOpen = false" :to="item.url" class="block py-1 text-sm text-gray-600">{{ item.label }}</router-link>
              </li>
            </ul>
          </li>

          <li>
            <router-link @click="mobileOpen = false" to="/contacts" class="block py-2 text-gray-600">CONTACT</router-link>
          </li>
        </ul>
      </div>
    </transition>
